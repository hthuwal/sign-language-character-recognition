...
def read_and_normalize_train_data():
    ...
    if not os.path.isfile(cache_path) or use_cache == 0:
        test_data = load_test()
        # cache_data((test_data, train_target, train_id, driver_id, unique_drivers), cache_path)
    else:
        print('Restore train from cache!')
        (test_data, train_target, train_id, driver_id, unique_drivers) = restore_data(cache_path)
...
def run_cross_validation_create_models(nfolds=10):
    ...
    test_data = read_and_normalize_train_data()
    start_time = time.time()
    model = load_model('save_vgg_model.h5')
    print('Load Model time: {} seconds'.format(round(time.time() - start_time, 2)))
    ...
    print('Prediction time: {} seconds'.format(round(time.time() - start_time, 2)))

    for i in range(0,len(guess)):
        cv2.putText(imgs[i],classfromlabel(guess[i]),(80,150),font,4,(255,255,255),2);
        # cv2.imshow(str(guess[i]),imgs[i]);
        name = str(i)+".png"
