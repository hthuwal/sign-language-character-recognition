...
class GridOption:
    ...
    def __init__(self, dataset_pathname, options):
        ...
        if sys.platform != 'win32':
            ...
            # example for windows
            self.svmtrain_pathname = os.path.join(
                dirname, r'..\windows\svm-train.exe')
    ...
    def parse_options(self, options):
        ...
        if not self.grid_with_c and not self.grid_with_g:
            raise ValueError(
                '-log2c and -log2g should not be null simultaneously')
...
def redraw(db, best_param, gnuplot, options, tofile=False):
    ...
    if tofile:
        gnuplot.write(
            b"set term png transparent small linewidth 2 medium enhanced\n")
        gnuplot.write("set output \"{0}\"\n".format(
            options.png_pathname.replace('\\', '\\\\')).encode())
    ...
    gnuplot.write("set xrange [{0}:{1}]\n".format(
        options.c_begin, options.c_end).encode())
    gnuplot.write("set yrange [{0}:{1}]\n".format(
        options.g_begin, options.g_end).encode())
...
def calculate_jobs(options):
    ...
    c_seq = permute_sequence(
        range_f(options.c_begin, options.c_end, options.c_step))
    g_seq = permute_sequence(
        range_f(options.g_begin, options.g_end, options.g_step))
...
def find_parameters(dataset_pathname, options=''):
    ...
    if options.gnuplot_pathname:
        gnuplot = Popen(options.gnuplot_pathname, stdin=PIPE,
                        stdout=PIPE, stderr=PIPE).stdin
    else:
        gnuplot = None
