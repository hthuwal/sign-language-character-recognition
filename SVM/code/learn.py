...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description='train a visual bag of words model')
    parser.add_argument('-d', help='path to the dataset',
                        required=False, default=DATASETPATH)
...
def get_categories(datasetpath):
    cat_paths = [files
                 # cat_paths ["../database/a","../database/b",....]
                 for files in glob(datasetpath + "/*")
                 if isdir(files)]
    cat_paths.sort()
    cats = [basename(cat_path)
            for cat_path in cat_paths]  # cats ["a","b","c","d"....]
    return cats


def get_imgfiles(path):
    all_files = []
    all_files.extend([join(path, basename(fname))  # doubt!!!!
...
if __name__ == '__main__':
    ...
    datasetpath = args.d  # path to the database
    # get categories from database ["a","b","c"......]
    cats = get_categories(datasetpath)
    ncats = len(cats)
    print "searching for folders at " + datasetpath
    if ncats < 1:
        raise ValueError('Only ' + str(ncats) +
                         ' categories found. Wrong path?')
    print "found following folders / categories:"
    print cats
    print "---------------------"
    all_files = []  # to hold a list of all files
    all_files_labels = {}  # to hold the label for each file
    all_features = {}  # dictionary to hold each file and corresponding features
    cat_label = {}  # dictionary to hold each categories file
    for cat, label in zip(cats, range(ncats)):
        # path to each category e.g ../database/a
    ...
    print "---------------------"
    print "## compute the visual words histograms for each image and saving the histogram"
    all_word_histgrams = {}
    ...
    pickle.dump(cat_label, open("dict"+".p", "wb"))
